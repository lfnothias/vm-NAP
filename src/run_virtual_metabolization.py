...
#Ensure the 'sygma' package is installed as it's crucial for GNPS data processing
...
def export_for_SIRIUS(input_df_name, compound_name):
    """
    Export metabolite data for SIRIUS processing.
    Handles data differently based on the source (SyGMa or BioTransformer).
    Drops duplicate entries and exports the resulting DataFrame to a TSV file.
    """
...
def export_for_NAP(input_df_name, compound_name):
    """
    Export metabolite data for NAP processing.
    Handles data differently based on the source (SyGMa or BioTransformer).
    Drops duplicate entries and exports the resulting DataFrame to a TSV file.
    """
...
def run_sygma_batch(list_smiles, list_compound_name, phase_1_cycle, phase_2_cycle, top_sygma_candidates, output_name, compound_name):
    """
    Process a list of SMILES strings through SyGMa to predict metabolites.
    Handles metabolite processing in batches and exports the results.
    Also, prepares the data for further processing by SIRIUS and NAP.
    """
...
def check_java_version():
    """Checks the current Java version to ensure compatibility with other components like BioTransformer."""
...
def download_and_unzip_biotransformer():
    """Downloads and extracts the BioTransformer tool if not already present. Ensures the necessary software is available for metabolite prediction."""
...
def prepare_environment(source_dir, dest_dir):
    """Prepares the working environment by moving files and renaming README to avoid conflicts with the current setup."""
...
def validate_biotransformation_type(type_of_biotransformation):
    """Validates if the provided biotransformation type is among the recognized ones."""
...
def create_or_clear_output_folder(folder_path):
    "Creates the output folder if it doesn't exist or clears it if it does, preparing it for new output."
...
def prepare_for_bio3(type_of_biotransformation, list_smiles):
    """
    Prepares the environment for BioTransformer processing.
    Includes checking Java version, setting up BioTransformer, and validating inputs.
    Ensures all prerequisites are met before starting the metabolite prediction.
    """
...
def run_biotransformer3(mode, list_smiles, list_compound_name, type_of_biotransformation, number_of_steps, output_name):
    """
    Runs BioTransformer for a list of SMILES strings to predict metabolites.
    Handles the execution of BioTransformer and processes the output, 
